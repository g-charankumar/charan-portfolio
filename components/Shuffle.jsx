...
// Fallback for SplitText if it's not available
let GSAPSplitText;
try {
  GSAPSplitText = require('gsap/SplitText').SplitText;
} catch (e) {
  // Simple manual splitting fallback if GSAP Premium is not available
  GSAPSplitText = class {
    constructor(el, vars) {
      this.el = el;
      ...
      this.chars = text.split('').map(char => {
        ...
        if (vars.charsClass) span.className = vars.charsClass;
        el.appendChild(span);
        return span;
      });
    }
...
const Shuffle = ({
  ...
  useGSAP(
    () => {
      if (!ref.current || !text || !fontsLoaded || isLoading) return;
